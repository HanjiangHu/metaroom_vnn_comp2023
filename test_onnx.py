...
if __name__ == '__main__':
    ...
    parser.add_argument("--onnx_file", type=str, help="output onnx")
    args = parser.parse_args()

    torch.set_num_threads(4)

    ort_session = custom_ort_session(args.onnx_file)

    # batch size is 1
    input_camera_motion = np.array([[0.0]]).astype(np.float32)
    ort_inputs = {"input": input_camera_motion}
    ort_outs = ort_session.run(None, ort_inputs)

    print(f"Input relative camera motion: {input_camera_motion[0]}, ground truth label: {int(args.onnx_file.split('_')[3][:-5])}, predicted label: {torch.argmax(torch.tensor(ort_outs[0]))}")
    print("ONNX model has been tested with ONNXRuntime with the custom operator, and the result looks good!")
